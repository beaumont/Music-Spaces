...
# Album with attached downloadable files
class FolderWithDownloadables < BasicFolderWithDownloadables

  delegate :number_of_tracks, :create_terms_acceptance, :to => :fwd_details_if_needed

  def set_specific_params_before_saving(params)
    if params[:terms] && params[:terms][:require_terms_acceptance] && !params[:terms][:require_terms_acceptance].to_i.zero?
      self.require_terms = true
    end

    TermsAndConditions.create_or_update!(params[:terms], fwd_details_if_needed)
  end

  def update_specific_params(params)
    save_contribution_settings(params)
  end

  def need_terms_acceptance_of(user)
    self.user.fwd_tos_allowed? && fwd_details_if_needed.need_terms_acceptance_of(user)
  end
end
