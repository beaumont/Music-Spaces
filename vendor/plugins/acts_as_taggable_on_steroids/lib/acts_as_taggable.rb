    ...
    module Taggable #:nodoc:
      ...
      module SingletonMethods
        ...
        def count_tagged_with(*args)
          ...
          options[:select] = "#{table_name}.id"
          options.blank? ? [] : count(options)
        end
        ...
        def find_options_for_find_tagged_with(tags, options = {})
          ...
          options.delete(:match_all) if tags.size == 1
          ...
          { :select => "DISTINCT #{table_name}.*", # removed DISTINCT
            ...
            #:group => "#{table_name}.id" # wtf ?? todo - figure out how to dedupe
          }.update(options)
        end
