  ...
  class Filter
    ...
    class << self
      ...
      def start!(controller)
        each_with_rescue(@@notes.flatten) do |note|
          klass = eval("Footnotes::Notes::#{note.to_s.camelize}Note", binding, __FILE__, __LINE__) if note.is_a?(Symbol) || note.is_a?(String)
    ...
    protected
      ...
      def initialize_notes!
        each_with_rescue(@@notes.flatten) do |note|
          note = eval("Footnotes::Notes::#{note.to_s.camelize}Note", binding, __FILE__, __LINE__).new(@controller) if note.is_a?(Symbol) || note.is_a?(String)
