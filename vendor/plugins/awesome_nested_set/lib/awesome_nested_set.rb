    ...
    module NestedSet #:nodoc:
      ...
      module SingletonMethods
        ...
        def acts_as_nested_set(options = {})
          ...
          named_scope :roots, lambda { |*opts| opts = opts.blank? ? nil : opts[0]; {:conditions => {parent_column_name => nil}.merge(opts || {}), :order => quoted_left_column_name} }
          named_scope :leaves, :conditions => "#{quoted_right_column_name} - #{quoted_left_column_name} = 1", :order => quoted_left_column_name
          named_scope :belonging_to, lambda { |parent| {:conditions => {:commentable_id => parent.id, :commentable_type => parent.class.name.to_s}, :order => quoted_left_column_name}  }
        end
