    ...
    module Backends
      ...
      module S3Backend
        ...
        def self.included(base) #:nodoc:
          mattr_reader :bucket_name, :cloud_front, :s3_config
          ...
          @@cloud_front = s3_config[:cloud_front]
        ...
        def full_filename(thumbnail = nil, options = {})
        ...
        def s3_url(thumbnail = nil)
          if cloud_front.blank?
            File.join(s3_protocol + s3_hostname + s3_port_string, bucket_name, full_filename(thumbnail, :escape => true))
          else
            File.join(s3_protocol + cloud_front + '.cloudfront.net', full_filename(thumbnail, :escape => true))
          end
        end
        ...
        protected
          ...
          def rename_file
            ...
            require 'cgi'
            old_full_filename = File.join(base_path, CGI::escape(@old_filename))
          ...
          def save_to_storage
            if save_attachment?
              #a hack to get it running on windows from http://nhw.pl/wp/2008/02/05/why-are-you-doomed-being-ruby-developer-on-windows-platform
              #somehow when You give file handle it won't read it
              if RUBY_PLATFORM =~ /(:?mswin|mingw)/
                file_content  = nil
                File.open(temp_path, 'rb') {|f| file_content = f.read }
              else
                file_content = temp_path ? File.open(temp_path) : temp_data
              end
              if attachment_options[:write_content_type]
                written_content_type = attachment_options[:write_content_type].call(self)
              else
                written_content_type = content_type
              end
              S3Object.store(
                full_filename,
                file_content,
              ...
              file_content.close if file_content.is_a?(File)
            end
