...
Sahi.prototype._byXPath = function (xpath, inEl) {
	...
	if (inEl){
		...
		var ix = this.findInArray(doc.getElementsByTagName(tagName), inEl);
...
//	this._alert(this.trim(expected) + " "  +  this.trim(actual));
...
Sahi.prototype._getTextNoTrim = function (el) {
    ...
    if (this.isIE() || this.isSafariLike()) return el.innerText;
...
Sahi.prototype.getDoc = function(relations){
	if (this.isArray(relations)){
		...
		for (var i=0; i<relations.length; i++){
			...
			if (!relation.type && relation.document) {
				nodes.push(relation.document); // window
			}
			if (relation.type && relation.type == "_near"){
//				this._alert(this.getDoc(relation).nodes.length);
		...
		if (inEl == null) new SahiDocProxy(nodes); 
		if (nodes.length == 0) return inEl;
//		this._alert(nodes.length);
		var narrowed = [];
		for (var i=0; i<nodes.length; i++){
//			this._alert(this._getText(inEl) + " ||| " + this._getText(nodes[i]));
...
//	alert(this._style(el, "height"));
	this.belowY = this.findPosY(el) + parseInt(this._style(el, "height"));
	return null;
};
...
Sahi.prototype.getDomRelAr = function(args){
	...
	for (var i=1; i<args.length; i++){
		if (args[i] && (args[i].isRelation == true || args[i].document)) rels.push(args[i]);
	}
	return rels;
}
